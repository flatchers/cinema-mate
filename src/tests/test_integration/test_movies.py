...
async def test_write_comments(client, db_session):
    ...
    stmt = select(Comment).where(Comment.user_id == user.id)
    result: Result = await db_session.execute(stmt)
...
async def test_add_and_remove_favourite(client, db_session):
    ...
    assert response_data_rem["message"] == "remove from favourite"
...
async def test_rate(client, db_session):
    ...
    assert response.status_code == 201
    response_data = response.json()
    assert response_data["message"] == f"updated rate - {payload["score"]}"

    stmt = select(Rate).where(Rate.movie_id == response_data_movie["id"])
    result: Result = await db_session.execute(stmt)
