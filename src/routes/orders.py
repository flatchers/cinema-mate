...
async def order_delete(
    ...
    stmt = select(OrderModel).where(OrderModel.id == order_id, OrderModel.user_id == current_user.id)
    ...
    if order.status != StatusEnum.PENDING:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Cannot delete order with status: {order.status.value}")
